...
import { deactivateInvite, resetInterviewStatus } from '@/lib/firebase/invites'
...
export const DeleteCandidateDialog: FC<DeleteCandidateDialogProps> = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Step 1: If we have an inviteId, deactivate the invite link and reset interview status
      if (inviteId) {
        console.log('Deactivating invite and resetting interview status:', inviteId);
        
        // Deactivate invite link
        const deactivateResult = await deactivateInvite(inviteId, auth.currentUser.uid)
        if (!deactivateResult.success) {
          throw new Error(deactivateResult.error || 'Failed to deactivate invite link')
        }
        
        // Reset interview status on backend
        const resetResult = await resetInterviewStatus(inviteId, auth.currentUser.uid)
        if (!resetResult.success) {
          console.warn('Failed to reset interview status:', resetResult.error);
