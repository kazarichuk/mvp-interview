...
export default function useInterviewSession({ sessionId }: UseInterviewSessionProps) {
  ...
  const RECOVERY_DELAY = 2000; // 2 seconds
  ...
  const fetchInterviewInfo = useCallback(async () => {
    try {
      console.log('Fetching interview info...');
      setLoading(true);
      setError(null);
      ...
      if (recoveryAttempts < MAX_RECOVERY_ATTEMPTS) {
        ...
        // Exponential backoff
        await new Promise(resolve => setTimeout(resolve, RECOVERY_DELAY * Math.pow(2, recoveryAttempts)));
        ...
        // Try to load from saved state as last resort
        const savedState = loadSessionState();
        if (savedState) {
          console.log('Loading from saved state...');
          setCurrentQuestion(savedState.currentQuestion);
          setCurrentTopic(savedState.currentTopic);
          setIsInitialized(true);
        }
      }
    }
  }, [sessionId, recoveryAttempts, loadSessionState]);
  ...
  const processInterviewResponse = useCallback((data: any) => {
    try {
      ...
      if (data.next_question) {
        ...
        setTimeout(() => {
          // Clear previous AI response first
          setAiResponse('');
          // Then update the question and topic
          setCurrentQuestion(data.next_question);
          setCurrentTopic(data.next_topic || null);
          // Save state after updating question
          saveSessionState();
